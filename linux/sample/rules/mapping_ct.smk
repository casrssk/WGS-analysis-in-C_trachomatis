...
rule  mapping_ct:
	input: 	
		r1="results/normalized/{sample}.norm_1.fastq.gz" if config["normalization"]==1 else "results/ct_fastq/{sample}_1.fastq.gz",
		r2="results/normalized/{sample}.norm_2.fastq.gz" if config["normalization"]==1 else "results/ct_fastq/{sample}_1.fastq.gz"
...
rule validate_bam:
	...
	shell:
		"""
		qualimap bamqc -bam {input} --outdir {params.outdir} --java-mem-size=8G
		"""
